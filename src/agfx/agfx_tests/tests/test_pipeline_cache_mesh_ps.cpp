...
// TESTS_TODO: "pipeline cache: MS + PS".
//
// Same recipe as test_pipeline_cache_vs_ps.cpp, but through a mesh pipeline (meshShader +
// fragmentShader, no task stage): pipeline A draws the left triangle via agfxRenderPassDrawMesh,
// agfxRenderPipelineGetCache pulls its blob, pipeline B is built from that blob alone and draws the
// right triangle. Mesh shading is optional, so this skips rather than fails where the device does
// not report support -- see mesh_common.h.
...
namespace
{
    ...
    constexpr const char* kGolden = "pipeline_cache_mesh_ps.png";
                                              ...
                                              agfxShaderModule* ps)
    {
        ...
        info.name = "pipeline cache ms+ps";
...
AGFX_TEST_TEXTURE(PipelineCacheMeshPs, C, kPipelineCacheWidth, kPipelineCacheHeight)
{
    ...
    AGFX_EXPECT_MSG(msShader.Valid() && psShader.Valid(), "failed to compile pipeline_cache.hlsl");
    ...
    const agfxRenderPipelineCreateInfo pipelineInfoA = PipelineInfo(msShader, msModuleA, psModuleA);
    ...
    agfxRenderPipelineCreateInfo pipelineInfoB = PipelineInfo(msShader, msModuleB, psModuleB);
...
AGFX_TEST_TEXTURE(PipelineCacheMeshPs, Cpp, kPipelineCacheWidth, kPipelineCacheHeight)
{
    if (!DeviceSupportsMeshShaders()) {
        ctx.Skip("device reports no mesh shader support");
        return;
    ...
    agfx::RenderPipeline pipelineA;
    {
        ...
        pipelineA = device.CreateRenderPipeline(PipelineInfo(msShader, msModule, psModule));
    }
    ...
    agfx::RenderPipeline pipelineB;
    {
        ...
        agfx::ShaderModule psModule(device.Get(),
            CreateShaderModule(device.Get(), psShader, "main_ps", AGFX_SHADER_MODULE_TYPE_FRAGMENT));
        agfxRenderPipelineCreateInfo pipelineInfoB = PipelineInfo(msShader, msModule, psModule);
        pipelineInfoB.cache = const_cast<uint8_t*>(cache.data());
        pipelineInfoB.cacheSize = cache.size();
...
AGFX_TEST_TEXTURE(PipelineCacheMeshPs, Ez, kPipelineCacheWidth, kPipelineCacheHeight)
{
    if (!DeviceSupportsMeshShaders()) {
        ctx.Skip("device reports no mesh shader support");
        return;
