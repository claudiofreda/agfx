...
// TESTS_TODO: "pipeline cache: TS + MS only".
//
// Same recipe as test_pipeline_cache_mesh_only.cpp, but with a task (amplification) shader in front
// of the mesh stage and no fragment shader: pipeline A dispatches one task group -- which itself
// calls DispatchMesh(1,1,1,...) -- writing the left triangle's depth; agfxRenderPipelineGetCache
// pulls its blob; pipeline B is built from that blob alone and writes the right triangle's depth.
// Revealed via the same depth-to-color compute sample as the other depth-only tests. Mesh shading is
// optional, so this skips rather than fails where unsupported -- see mesh_common.h.
...
namespace
{
    ...
    constexpr const char* kGolden = "pipeline_cache_task_mesh_only.hdr";
    ...
    agfxRenderPipelineCreateInfo PipelineInfo(const CompiledShader& meshShader, const CompiledShader& taskShader,
                                              agfxShaderModule* ms, agfxShaderModule* as)
    {
        agfxRenderPipelineCreateInfo info{};
        info.name = "pipeline cache ts+ms only";
...
AGFX_TEST_TEXTURE(PipelineCacheTaskMeshOnly, C, kPipelineCacheWidth, kPipelineCacheHeight)
{
    ...
    AGFX_EXPECT_MSG(msShader.Valid() && asShader.Valid(), "failed to compile pipeline_cache.hlsl");
    ...
    const agfxRenderPipelineCreateInfo pipelineInfoA = PipelineInfo(msShader, asShader, msModuleA, asModuleA);
    ...
    agfxRenderPipelineCreateInfo pipelineInfoB = PipelineInfo(msShader, asShader, msModuleB, asModuleB);
...
AGFX_TEST_TEXTURE(PipelineCacheTaskMeshOnly, Cpp, kPipelineCacheWidth, kPipelineCacheHeight)
{
    if (!DeviceSupportsMeshShaders()) {
        ctx.Skip("device reports no mesh shader support");
        return;
    ...
    agfx::RenderPipeline pipelineA;
    {
        ...
        pipelineA = device.CreateRenderPipeline(PipelineInfo(msShader, asShader, msModule, asModule));
    }
    ...
    agfx::RenderPipeline pipelineB;
    {
        ...
        agfx::ShaderModule asModule(device.Get(),
            CreateShaderModule(device.Get(), asShader, "main_as", AGFX_SHADER_MODULE_TYPE_TASK));
        agfxRenderPipelineCreateInfo pipelineInfoB = PipelineInfo(msShader, asShader, msModule, asModule);
        pipelineInfoB.cache = const_cast<uint8_t*>(cache.data());
        pipelineInfoB.cacheSize = cache.size();
...
AGFX_TEST_TEXTURE(PipelineCacheTaskMeshOnly, Ez, kPipelineCacheWidth, kPipelineCacheHeight)
{
    if (!DeviceSupportsMeshShaders()) {
        ctx.Skip("device reports no mesh shader support");
        return;
